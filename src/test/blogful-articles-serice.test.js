...
describe('Articles Service', function () {
  ...
  let testArticles = [
    {
      id: 1,
      date_published: new Date('2020-10-25 09:36:38.078848-07'),
  ...
  context('getAllArticles()', () => {
    beforeEach(async () => {
      await db('blogful_articles').insert(testArticles);
    });

    it('getAllArticles() resolves all articles from blogful_articles table', async () => {
      const articles = await ArticlesService.getAllArticles(db);
      return expect(articles).to.eql(testArticles);
    });

    it('insertArticle() inserts a new article and resolves the new article with an id', async () => {
      const newArticle = {
        id: 4,
      ...
      const res = await ArticlesService.insertArticle(db, newArticle);
      return expect(res).to.eql(newArticle);
    });
